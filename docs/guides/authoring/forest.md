...
## What exists today
- A small TEST forest patch in village_21 around **(90, 225)** (chunk 2_7): oaks, pines,
  bushes, one wild `wasp_nest` deep in the patch, and the centipede spawn circle
  (zone.json spawn_areas `forest_patch`, cap 2).
- The wasp-pen vignette near the fly farm at **(124, 232)**: a wild nest beside the
  stone path with wood OBSERVATION fencing (deliberately wood: the lesson is that
  fences do NOT stop wings) and flowers.
