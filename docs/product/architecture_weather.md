...
- **Client visuals** (`RainController`, created by DayNightController): camera-following
  stretched streak particles + ambient dim ×0.85 + ☔ on the clock.

## Day/night display (DayNightController)

- Curve: day 0-0.42 → smoothstep golden dusk 0.42-0.58 → DEEP night floor 0.20
  (you genuinely can't see past light radii) 0.58-0.88 → smoothstep dawn 0.88-1.0.
- The fruit-fall evening window t∈[0.40,0.62) sits inside the dusk ramp by design —
  trees shed while the light goes golden.
- Clock UI: `☀/☽/☔ Day N HH:MM`, center-top. F7 = client-only lighting preview.
- Personal light (`PlayerNightLight`, all data-driven — no hardcoded item ids):
  equipped item with a `world.light` block → its held-glow; tool_type "flashlight" →
  base glow + a Light2D CONE (radius 8, 30°/70°) aimed at the mouse; else a dim
  radius-2.5 glow. Underground full-dark = a zone ambient flag (BACKLOG).
