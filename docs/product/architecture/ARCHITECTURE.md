...
- [architecture_shaped_ground.md](architecture_shaped_ground.md) — the shovel terraform builder: composite `matA~matB~shape` ground ids, runtime GPU mask-composite, the primary-material rule, dig↔place material loop
- [architecture_bugs.md](architecture_bugs.md) — bug species, AI, lifecycle
- [architecture_combat.md](architecture_combat.md) — **DESIGN** combat AI & challenge: attack-token stage manager + FSM brains + steering (adopted), + the recommended layers (three-phase telegraphs, player dodge/i-frames, threat/aggro/leash, threat director) and the deterministic/network model
...
├── tools/                          # Python sprite/world pipeline
└── docs/                           # product/ (how the game works) + guides/ (pipeline)
```
