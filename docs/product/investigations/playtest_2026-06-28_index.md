...
| 5 | Unable to sell items (tried clicking different ways); can't test buying (no money) | INVESTIGATE | `cannot-sell-items` | ⚠ BLOCKED ON 1 confirm — sell system fully wired; likely the **shop won't open** due to the shared single-`OverlapPoint` click resolution (NPCs overlap storefronts). Confirm: does the NPC dialogue open on right-click? |
...
| 14 | Basket + other containers don't seem functional — audit | INVESTIGATE | `container-functionality-audit` | ✅ READY — **basket/chest/trunk/yarn_basket** have a container block but **no `interaction_type:"storage"`** → never open (gate `CraftingPanel.cs:138`). Fix = add the tag + publish. (+shared OverlapPoint.) |
| 10 | Moving the compost container broke it — could no longer load compost | INVESTIGATE | `compost-broke-on-move` | ✅ READY — compost `StationState` is created only by the chunk-load scan; runtime-placed bins get none and `handleStationDeposit:958` requires one → "No station there". Fix = lazy find-or-create like `resolveCraftStation`. Sim-touching → determinism gate. |
