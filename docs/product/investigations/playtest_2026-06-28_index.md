...
| 2 | Placing a block (not everything) — preview is LOWER than where it actually lands | INVESTIGATE | `block-place-preview-offset` | ✅ FIXED (2026-07-02, `TilemapManager.OccupantWorldPos` shared helper — render + ghost use the same footprint-X/pivot-Y math; ghost uses the seed→plant id it renders). |
...
| 7 | Rain doesn't reach the bottom of the screen | KNOWN-FIX | — | ✅ FIXED (2026-07-02) — streak LIFETIME was a fixed spec shorter than a screen crossing; now computed from the live camera (slowest drop crosses the bottom edge + margin), band/splash boxes sized from the view, rebuilt on zoom. |
