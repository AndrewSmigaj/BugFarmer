...
- **★ DETERMINISM BUG — spawn-apart (disjoint-chunk) late-join DIVERGES ~11%.** With the read-cap fix the late-joiner now bootstraps, so the canonical gate's 2nd half (`SPAWN_A=126,2 SPAWN_B=126,253`, disjoint chunks) finally runs — and shows ~11% per-bug divergence / hash-mismatch every tick, while **co-located is byte-identical**. So a deterministic-sim input is read **chunk-scoped (view-dependent)**, not zone-wide (the #133 class). **NOT** the dead-bugs change (co-located clean) and **NOT** the collision map (verified sent to late-joiners at `match.go:584`). Was masked ~a week (read-cap bug made the late-joiner 0-swarm → spawn-apart untestable). Needs a focused frontier-sync dive: instrument what diverges FIRST at the join boundary (prime suspects: a player-position/player-cell on-receipt read, or a chunk-gated influence/food read) → pin → fix → re-verify BOTH gate halves IDENTICAL. Also reconcile `architecture_swarm_sync.md` (add the client read-cap constraint + this known-issue).
