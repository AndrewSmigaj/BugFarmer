...
def build():
    ...
    for lp in [(7, 5), (9.5, 6), (8, 4)]:
        b.place_decor("water_lily", lp[0], lp[1])
    ...
    for fid, dx, dy in ring:
        b.place_decor(fid, cx + 0.5 + dx, cy + 0.5 + dy)
    ...
    for _ in range(5):
        b.place_bug("butterfly_monarch", rng.uniform(33, 44), rng.uniform(23, 32))
    ...
    b.place_bug("butterfly_swallowtail", 50.5, 26.5)
    b.place_bug("ladybug_orange", 52, 25.5)
    ...
    for gx in range(8, 19, 2):                          # poppies/weeds along the old fence
        b.place_decor("poppy", gx + 0.4, 36.6)
    b.place_bug("spider_orb", 13, 36.4)      # a web strung between the rails
    b.place_bug("butterfly_swallowtail", 16, 36.8)
    ...
    b.place_bug("moth_brown", 57.2, 43.6)    # moths at the lantern
    ...
    flower_patch(b, 38, 32, W - 1, 50, ["poppy", "flower_red", "flower_yellow", "dandelion"], 16,
                 seed=32)
    flower_patch(b, 0, 1, W - 1, H - 1, fkinds, 26, seed=3)

    # --- DRIFTING POLLINATORS + the wasps (predation pressure) across the whole meadow ---
    for _ in range(4):
        b.place_bug("butterfly_monarch", rng.uniform(2, 60), rng.uniform(2, 48))
    for _ in range(3):
        b.place_bug("butterfly_swallowtail", rng.uniform(2, 60), rng.uniform(2, 48))
    for _ in range(3):
        b.place_bug("butterfly_common", rng.uniform(2, 60), rng.uniform(2, 48))
    for _ in range(4):
        b.place_bug(rng.choice(["bumblebee", "honeybee"]),
                    rng.uniform(2, 60), rng.uniform(2, 48))
    b.place_bug("wasp_paper", 46, 44)        # the easier wasp, near the camp/oak
    b.place_bug("wasp_yellowjacket", 36, 31)  # the harder wasp, hunting the milkweed swarm
