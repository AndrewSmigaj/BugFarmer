...
namespace BugFarmer.World
{
    ...
    public static class BlobShadow
    {
                                  ...
                                  Vector2 parentScale, float alpha)
        {
            ...
            float w = Mathf.Max(0.6f, widthCells) * 1.15f;  // world units (cellSize=1); a touch wider than base
            float h = w * 0.35f;                             // flat ellipse
            ...
            // Parent scale multiplies localPosition, so counter sy to land exactly on the base.
            sr.transform.localPosition = new Vector3(0f, (-0.5f * spriteHeightCells) / sy, 0.01f);
        }
