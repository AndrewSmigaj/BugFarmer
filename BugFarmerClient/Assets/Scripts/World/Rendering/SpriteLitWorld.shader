// BugFarmer world sprite shader: a copy of URP's 2D "Sprite-Lit-Default" + two additions —
//   (1) vertex WIND sway (base-anchored, world-phased) for foliage; strength 0 = no sway (default).
//   (2) a fragment FLASH lerp-to-color for hit feedback (driven per-renderer via a MaterialPropertyBlock).
// All three passes get the 4 extra props (SRP-batcher needs matching UnityPerMaterial layouts).
Shader "BugFarmer/SpriteLitWorld"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 CombinedShapeLightFragment(Varyings i) : SV_Target
            {
                ...
                litColor.rgb = lerp(litColor.rgb, _FlashColor.rgb, _FlashAmount);   // hit-flash (per-renderer MPB)
                return litColor;
            }
            ENDHLSL
        }
